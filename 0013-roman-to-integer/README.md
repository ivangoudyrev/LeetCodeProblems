Solution notes: A bit clunky.  First for-loop to find and then replace substring elements for special substrings ("IV", "IX", etc.). Also the while loop that re-runs the second for-loop multiple times.  Need to optimize later.
